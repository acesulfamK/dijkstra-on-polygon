...
void DijkstraPQ(int p, int size)
{
	...
	while (!PQ_Empty())
	{
		...
		for (n = list[u]; n != NULL; n = n->next /* For all adjacent nodes from Node-u */)
		{
			int w = n->weight;
			int newlen = dist[u] + w;
			if (newlen < dist[n->id] /* newlen is shorter than dist[n->id] */)
			{
				dist[n->id] = newlen; /* Update dist[n->id] */
				prev[n->id] = u;	  /* Update prev[n->id] */
...
void Min_Path(int s, int p, int size)
{
